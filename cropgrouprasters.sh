...
for crop_group in $(cat crop_groups.txt); do
  ...
  grep $crop_group earthstat_crops.tsv | 
    awk '{print "/nfs/qread-data/raw_data/landuse/global_aglands/HarvestedAreaYield175Crops_Geotiff/HarvestedAreaYield175Crops_Geotiff/"$1"_HarvAreaYield_Geotiff/"$1"_HarvestedAreaFraction.tif"}' > crop_names.txt
